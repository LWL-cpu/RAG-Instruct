...
#     query = str(Question) 
#     retrieval_docs = retriever.search_document(query, n_docs)  # 检索文档
...
def load_items(file_path):
    ...
    for key, values in data.items():
        for item in values:
            item['source'] = key  # 将原键名作为新字段添加到每个item
            input_data.append(item)  # 添加到输入数据列表中
...
def main(args):
    ...
    file_path = args.input_name  # 数据路径
